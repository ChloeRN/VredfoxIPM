...
Nater, C. R, Hofhuis, S. P., Grainger, M., Flagstad, Ø., Ims, R. A., Killengreen S., Ehrich D. (2025).
An integrated population modelling workflow for supporting mesopredator management. *EcoEvoRxiv*. 
DOI: <https://doi.org/10.32942/X2Z33G>
...
The relevant data tables are called "V_redfox_carcass_examination" (version 4: <https://data.coat.no/dataset/v_redfox_carcass_examination_v4>), "V_redfox_hunting" (version 3: <https://data.coat.no/dataset/v_redfox_hunting_v3>) and "V_rodents_snaptrapping_abundance_regional" (version 7, <https://data.coat.no/dataset/v_rodents_snaptrapping_abundance_regional_v7>). The datasets that are under embargo; access tokens can be requested through Dr. Dorothée Ehrich (dorothee.ehrich@uit.no). 
...
pkg.list <- c("ckanr", "coda", "cowplot", "dplyr", "ggplot2", "ggspatial", "MCMCvis", "metafor", "patchwork", "purr", "remotes", "reshape2", "sf", "tidyterra", "truncnorm")
...
The red fox workflow contains a modular analysis, meaning that there are a range of toggles ("switches") that can be used to customize model structure and assumptions. The first set of toggles deals with the use of environmental covariates and temporal random effects fit on different vital rates:
```{r, eval = FALSE}
# Covariate toggles
...
The next set deal with the inclusion of summer harvest data, specifically whether age information should be used and for which years summer age-at-harvest data should be included. In our main analysis, we do not use age information from summer harvest as we suspect these to be biased (not representative of the population). Instead, we will treat all individuals harvested in summer as "unaged" and add their numbers to the main (winter) harvest counts, assuming a similar age distribution as in the winter harvest. If we wanted to include age-at-harvest data from summer separately, it may be best to only use years 2005-2012, as we know that aging became very biased after that.
```{r, eval = FALSE}
...
The next four sets of toggles are related to four key aspects of our integrated population model (IPM): the informative priors for natural mortality / annual survival, the way genetic data is included to inform immigration rate, the decision to include opportunistic survey data (or not), and the modelling of density-dependence and compensatory mechanisms.
...
Finally, our setup provides the possibility to include density-dependence and/or compensatory mechanisms that affect either natural mortality, immigration, or both: 
```{r, eval = FALSE}
...
For density-dependence, the third toggle (`DDxRodent`) determines whether or not an interactive effect of population density and rodent abundance should be fit in addition to single effects. 
For compensation, the third toggle (`comp.RE`) determines how harvest compensation is modelled. If `comp.RE = FALSE`, it is modelled as a fixed effect of the log deviance of year-specific harvest mortality (`= log(mH[a,t]) - log(Mu.mH[a]))`). If `comp.RE = TRUE`, a correlated random effect is used instead. 
...
Another important argument to `setupModel()` is `testRun`. If this is set to TRUE, it will set up for a model test run with only 10 iterations, no thinning, and no burn-in. Otherwise, it will default to 200000 iterations with a burn-in of 75000 and a thinning factor of 20, but these can be changed by manually passing them to the function (see function documentation). The same is true for the number of chains, which defaults to 4 unless otherwise specified. 
...
A full model run will take on average 3-6 hours depending on the machine you run it on. 
...
The primary outputs of the fitted IPM are posterior samples for averages and time-series of vital rates and population sizes, as well as environmental effects and other quantities describing time-variation in population dynamics. The workflow contains one function for summarising these numerically and two functions for visualizing these results in a variety of different ways:
```{r, eval = FALSE}
...
## Follow-up analyses
The posterior samples from the fitted IPM become the input for a few different follow-up analyses, including 
transient Life Table Response Experiment (tLTRES), variance decompositions, and posterior correlations. We also present the setup for Population Viability Analyses (PVAs) here, even though these do not use posterior samples as an input but are instead integrated in the estimation procedure (see below).
For more background and details on the different analyses, please refer to the preprint / article, and the function documentation. 

### Transient Life Table Response Experiments (tLTREs)
tLTREs are tools to quantify the relative importance of changes in different vital rates and population structure to changes in population dynamics, specifically realized population growth rates. There are two different approaches: the random design, which focuses on variation across the entire study period, and the fixed design, which focuses on year-by-year changes. Both require calculation of transient sensitivities first, followed by the calculation of LTRE contributions. The two toggles in the code below are used to determine whether the decomposition i) should be done at the level of survival probabilities or mortality hazard rates (`HazardRates`) and ii) focus on population age class proportions or absolute number in each age class (`PopStructure`). The workflow both calculates and visualizes sensitivities and LTRE contributions.
```{r, eval = FALSE}
...
In the original study we explicitly accounted for density-dependence in natural mortality of the first age class and immigration rates, as well as potential correlation between natural and harvest mortality. Post-hoc correlations of IPM posteriors can give some insights into potential additional mechanisms at play and the following function does this for a selection of parameter pairs: 
```{r, eval = FALSE}
...
```
We do note that this approach to testing for density-dependence and demographic compensation is not ideal; while practical and straightforward to implement, correlations made based on posterior samples will always be a mixture of true "process correlation" and MCMC sampling correlations. This should always be considered when interpreting results. 
...
Note that as of yet, the PVAs are not integrated into the targets workflow.
