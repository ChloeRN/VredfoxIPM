...
Our study also included Population Viability Analyses (PVAs), which are forward-projections of the population model beyond the duration of the study period. We implemented the PVAs as fully integrated, i.e. PVA are performed as the models are fitted. As such, fitting the models used for PVA was done using alternative code that can be found on another branch of the repository called "PVA_basicSetup" (<https://github.com/ChloeRN/VredfoxIPM/tree/PVA_basicSetup>). 
In essence, the PVA version of the workflow includes a variable `Tmax_sim`, which determines how many years beyond the study period the model should run, as well as a range of "perturbation factors" for vital rates. The perturbation factors can be set to reflect potential management scenarios that the user envisions. Our current implementation supports both direct a-priori manipulaiton of annual perturbation factors and perturbation factors that respond dynamically to predicted small rodent abundances.

As of yet, the PVA analyses are not fully integrated into the targets workflow.
