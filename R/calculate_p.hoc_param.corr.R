...
calculate_p.hoc_param.corr <- function(MCMC.samples, 
  ...
  corr_lambda <- corr_lambda_local <- corr_mH <- corr_mH_local <- corr_mO <- corr_mO_local <- corr_immR <- corr_Psi <- corr_rho <- rep(NA, nrow(out.mat))
  ...
  for(i in 1:nrow(out.mat)){
    ...
    densT_local <- unname(out.mat[i,  paste0("N.tot[", yearIdxs,"]")] - out.mat[i,  paste0("Imm[", yearIdxs,"]")])
    ...
    corr_lambda_local[i] <- cor.test(lambdaT[2:(Tmax-1)], densT_local[2:(Tmax-1)])$estimate        
    ...
    corr_mH_local[i] <- cor.test(mHT[2:Tmax], densT_local[2:Tmax])$estimate
    ...
    corr_mO_local[i] <- cor.test(mOT[2:Tmax], densT_local[2:Tmax])$estimate
    corr_immR[i] <- cor.test(immRT[2:Tmax], densT_local[2:Tmax])$estimate              #the first year pop size is set to 0 so remove the first year with indexing
    corr_Psi[i] <- cor.test(PsiT[2:Tmax], (densT_local[2:Tmax]- R.totT[2:Tmax]))$estimate   #N-R = adult population size, because reproduction should not depend on offspring that are not there yet
    corr_rho[i] <- cor.test(rhoT[2:Tmax], (densT_local[2:Tmax]- R.totT[2:Tmax]))$estimate   #N-R = adult population size, because reproduction should not depend on offspring that are not there yet)
  ...
  abline(v=0, col="blue")
  hist(corr_lambda_local, main = paste("Lambda ~ localN"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_mH, main = paste("Harvest ~ N"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_mH_local, main = paste("Harvest ~ localN"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_mO, main = paste("Nat.mort ~ N"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_mO_local, main = paste("Nat.mort ~ local N"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  
  par(mfrow = c(3, 1))
  
  hist(corr_immR, main = paste("Immigration ~ localN"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_Psi, main = paste("BreedingProb ~ localN"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  hist(corr_rho, main = paste("LitterSize ~ localN"), xlab=NULL, ylab=NULL)
  abline(v=0, col="blue")
  ...
  corr_data <- rbind(quantile(corr_lambda, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
                     quantile(corr_lambda_local, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
                     quantile(corr_mH, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
                     quantile(corr_mH_local, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
                     quantile(corr_mO, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
                     quantile(corr_mO_local, probs = c(0.5, 0.025, 0.25, 0.75, 0.975)),
    ...
    tibble::as_tibble() %>%
    dplyr::mutate(Parameter = c("Lambda ~ N", "Lambda ~ local N",
                                "Harvest ~ N", "Harvest ~ local N", 
                                "Nat.mort ~ N", "Natural mort. ~ local N", 
                                "Immigration ~ local N", "Breeding prob. ~ local N", "Litter size ~ local N", 
                                "Harvest ~ Natural mort.", "Harvest ~ Natural mort.[t+1]", 
                                "Harvest ~ Imm", "Harvest ~ Imm[t+1]", "Harvest ~ Reproduction[t+1]", 
                                "Natural mort. ~ Imm", "Natural mort. ~ Reproduction[t+1]"),
