...
#' population-level quantities. The sublist "t" contains time-specific 
...
extractParamSamples <- function(MCMC.samples, Amax, Tmax){
  ...
  N_mean <- apply(N[, , 1:Tmax], c(1, 2), mean)
  N_tot_mean <- rowMeans(N_tot)
  n_mean <- N_mean / N_tot_mean
  
  B_mean <- apply(B[, , 1:Tmax], c(1, 2), mean)
  B_tot_mean <- rowMeans(B_tot[, 1:Tmax])
  L_mean <- apply(L[, , 1:Tmax], c(1, 2), mean)
  L_tot_mean <- rowMeans(L_tot[, 1:Tmax])
  R_mean <- apply(R[, , 1:Tmax], c(1, 2), mean)
  R_tot_mean <- rowMeans(R_tot[, 1:Tmax])
  ...
  S_mean <- apply(S[, , 1:(Tmax-1)], c(1, 2), mean)
  mH_mean <- apply(mH[, , 1:(Tmax-1)], c(1, 2), mean)
  mO_mean <- apply(mO[, , 1:(Tmax-1)], c(1, 2), mean)
  ...
  lambda_mean <- rowMeans(lambda[, 1:(Tmax-1)], na.rm = T)
