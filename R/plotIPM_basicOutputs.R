  ...
  # Environmental covariates panel (Not very pretty coding but it works..)
    #  making a dataframe of what I want to plot, I have to add time manually because its not in input.data
     forplot <- data.frame(
      ...
      rodent_wint_time  = seq(as.Date("2004-07-01"), as.Date("2022-07-01"), by="+1 year"), #because no rodent yearinfo in input data i had to do this manually...
      ...
      rodent_fall_time  = seq(as.Date("2004-04-15"), as.Date("2022-04-15"), by="+1 year"),
      reindeer          = nim.data$Reindeer,
      reindeer_time     = seq(as.Date("2004-07-01"), as.Date("2022-07-01"), by="+1 year")
       )
    ...
    p.E_time <- ggplot(forplot)  +  
      ...
      scale_x_date(date_breaks = "1 year", date_labels = "%Y", minor_breaks =NULL, limits = c(as.Date("2003-12-31"), as.Date("2021-12-01")))+ #Here you would also have to edit the date range manually..
